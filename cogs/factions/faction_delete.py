...
class FactionDelete(commands.Cog):
    ...
    async def delete_faction(self, interaction: discord.Interaction, name: str):
        ...
        if (ch := guild.get_channel(int(faction["channel_id"]))):
            await ch.send(embed=make_embed("💀 Faction Disbanded", f"**{name}** has been officially disbanded."), delete_after=10)
            await ch.delete()
        ...
        await interaction.followup.send(embed=make_embed("✅ Faction Deleted", f"Faction **{name}** removed successfully.", 0xE74C3C), ephemeral=True)
