...
class FactionMembers(commands.Cog):
    ...
    async def add_member(self, interaction, faction_name: str, member: discord.Member):
        ...
        if str(member.id) in members:
            return await interaction.followup.send(f"⚠️ {member.mention} already in `{faction_name}`.", ephemeral=True)
    ...
    async def remove_member(self, interaction, faction_name: str, member: discord.Member):
        await interaction.response.defer(ephemeral=True)
        if not interaction.user.guild_permissions.administrator:
        ...
        if str(member.id) not in members:
            return await interaction.followup.send(f"⚠️ {member.mention} not in `{faction_name}`.", ephemeral=True)
