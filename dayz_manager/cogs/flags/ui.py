...
from dayz_manager.cogs.utils import database as db  # ✅ shared, live connection
...
class FlagManageView(View):
    ...
    async def assign_flag_button(self, interaction: discord.Interaction, _: discord.ui.Button):
        ...
        async with db.db_pool.acquire() as conn:
            all_flags = await conn.fetch(
                "SELECT flag, status FROM flags WHERE guild_id=$1 AND map=$2 ORDER BY flag ASC;",
                guild_id,
                self.map_key,
            )
        ...
        flag_options = [
            discord.SelectOption(label=f["flag"], value=f["flag"]) for f in unclaimed
        ]
        role_options = [
        ...
        async def confirm_assign(inter2: discord.Interaction):
            ...
            await inter2.edit_original_response(
                content=f"✅ Assigned `{flag_value}` to {role.mention}.", view=None
            )
        ...
        await interaction.followup.send(
            "Select a flag and role to assign:", view=confirm_view, ephemeral=True
        )
    ...
    async def release_flag_button(self, interaction: discord.Interaction, _: discord.ui.Button):
        ...
        flag_options = [
            discord.SelectOption(label=f["flag"], value=f["flag"]) for f in claimed
        ]
        ...
        async def confirm_release(inter2: discord.Interaction):
            ...
            async with db.db_pool.acquire() as conn:
                await conn.execute(
                    "UPDATE flags SET status='✅', role_id=NULL WHERE guild_id=$1 AND map=$2 AND flag=$3;",
                    guild_id,
                    self.map_key,
                    flag_value,
                )
            ...
            await inter2.edit_original_response(
                content=f"✅ Released `{flag_value}`.", view=None
            )
        ...
        await interaction.followup.send(
            "Select a flag to release:", view=release_view, ephemeral=True
        )
