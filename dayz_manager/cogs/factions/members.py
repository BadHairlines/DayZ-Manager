...
class FactionMembers(commands.Cog):
    ...
    async def add_member(self, interaction: discord.Interaction, faction_name: str, member: discord.Member):
        ...
        await log_faction_action(guild, action="Member Added", faction_name=faction["faction_name"], user=interaction.user, details=f"{interaction.user.mention} added {member.mention} to faction `{faction['faction_name']}` (Map: `{map_key}`).")
    ...
    async def remove_member(self, interaction: discord.Interaction, faction_name: str, member: discord.Member):
        ...
        await log_faction_action(guild, action="Member Removed", faction_name=faction["faction_name"], user=interaction.user, details=f"{interaction.user.mention} removed {member.mention} from faction `{faction['faction_name']}` (Map: `{map_key}`).")
