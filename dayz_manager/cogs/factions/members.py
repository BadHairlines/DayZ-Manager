...
from dayz_manager.cogs.utils import database as db  # ✅ live global db reference
...
class FactionMembers(commands.Cog):
    """Handles adding and removing faction members."""
    ...
    async def add_member(
        self, interaction: discord.Interaction, faction_name: str, member: discord.Member
    ):
        ...
        if not faction:
            return await interaction.followup.send(
                f"❌ Faction `{faction_name}` not found.", ephemeral=True
            )
    ...
    async def remove_member(
        ...
        if not interaction.user.guild_permissions.administrator:
            return await interaction.followup.send(
                "❌ Only admins can remove members.", ephemeral=True
            )
        ...
        if str(member.id) not in members:
            return await interaction.followup.send(
                f"⚠️ {member.mention} is not in `{faction_name}`.", ephemeral=True
            )
