...
from dayz_manager.cogs.utils import database as db  # ✅ live reference (no frozen pool)
...
class FactionCreate(commands.Cog):
    """Handles faction creation, flag claiming, and channel/role setup."""
    ...
    async def create_faction(
        ...
        if not interaction.user.guild_permissions.administrator:
            return await interaction.followup.send(
                "❌ Only admins can create factions.", ephemeral=True
            )
        ...
        if not flag_data:
            return await interaction.followup.send(
                f"🚫 Flag `{flag}` does not exist on `{map_key}`.", ephemeral=True
            )
        if flag_data["status"] == "❌":
        ...
        # 🧾 Insert into factions + update flag ownership
        async with db.db_pool.acquire() as conn:
            await conn.execute("""
                INSERT INTO factions (guild_id, map, faction_name, role_id, channel_id, leader_id, member_ids, color)
                VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
            """, guild_id, map_key, name, str(role.id), str(channel.id), str(leader.id),
                [str(m.id) for m in members], color.value)
        ...
        # 🪶 Log the creation
        await log_faction_action(
            guild,
            action="Faction Created + Flag Claimed",
            faction_name=name,
            user=interaction.user,
            details=(
                f"Leader: {leader.mention}, Map: {map.value}, Flag: {flag}, "
