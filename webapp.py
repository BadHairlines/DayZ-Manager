...
async def guild_dashboard_page(request: web.Request) -> web.Response:
    ...
    for row in sessions:
        ...
        setup_rows.append({"map": map_key, "map_name": map_name, "server": server})
    setup_rows.sort(key=lambda x: (x["map_name"].casefold(), x["server"].casefold()))
    setup_json = json.dumps(setup_rows).replace("<", "\\u003c")
    maps_json = json.dumps([{"key": key, "name": value["name"]} for key, value in utils.MAP_DATA.items()]).replace("<", "\\u003c")
    flags_json = json.dumps(list(utils.FLAGS)).replace("<", "\\u003c")
    csrf = json.dumps(str(session.get("csrf_token") or ""))

    icon = f'<img alt="" src="{html.escape(str(guild.icon.url))}">' if guild.icon else "🛡️"
    setup_list = "".join(
        f'<div class="setup-row"><div><strong>{html.escape(x["map_name"])} • {html.escape(x["server"])}</strong><div class="tiny">{html.escape(_setup_key(x["map"], x["server"]))}</div></div><a class="btn" href="{html.escape(flag_page_url(guild.id, x["map"], x["server"]) or "#")}">Live Page</a></div>'
        for x in setup_rows
    ) or '<div class="empty">No Flag Systems yet. Use Web /setup below.</div>'

    teleporter_html = ""
    if guild.id in set(ALLOWED_GUILD_IDS):
        teleporter_html = """
<section id="teleporter" class="card tool-card">
  <span class="web-command">/teleporter</span><h3>🌀 Teleporter Generator</h3>
  <p>Generate the same two-way DayZ teleporter JSON files available from the Discord command.</p>
  <form id="teleporterForm" class="form-grid">
    ...
    <div class="field full"><button class="btn primary" type="submit">Generate JSON Files</button></div>
  </form>
  <div class="result" id="teleporterResult"></div>
</section>"""

    body = f"""
<main class="wrap"><section class="section" style="padding-top:38px">
<a href="/dashboard" class="meta">← My Servers</a>
<div class="dashboard-head" style="margin-top:16px"><div><span class="private-badge">🔐 ADMIN CONTROL PANEL</span><h1 style="font-size:clamp(36px,6vw,62px);margin:9px 0">{html.escape(guild.name)}</h1><p class="section-sub">Every current DayZ Manager Discord command is available here as a web control.</p></div><div class="server-icon" style="width:70px;height:70px;font-size:32px">{icon}</div></div>
<div class="manage-shell" style="margin-top:24px">
  <aside class="card manage-side">
    <strong>Web Commands</strong>
    <a href="#setups">🗂️ /setups</a><a href="#setup">➕ /setup</a><a href="#assign">🏴 /assign</a><a href="#release">🏳️ /release</a><a href="#status">📊 /flagstatus</a><a href="#refresh">🔄 /flagrefresh</a><a href="#history">🕘 /flaghistory</a><a href="#delete">🗑️ /deletesetup</a><a href="#botstatus">🤖 /botstatus</a>
    {"<a href='#teleporter'>🌀 /teleporter</a>" if teleporter_html else ""}
  </aside>
  <div class="tool-stack">
    <section id="setups" class="card tool-card"><span class="web-command">/setups</span><h3>🗂️ Flag System Setups</h3><p>All Flag Systems currently configured in this Discord server.</p><div id="setupList" class="setup-table">{setup_list}</div></section>

    <section id="setup" class="card tool-card"><span class="web-command">/setup</span><h3>➕ Create / Repair Setup</h3><p>Create a Flag System channel/dashboard or repair an existing setup.</p><form id="setupForm" class="form-grid"><div class="field"><label>Map</label><select class="select" name="map" data-map-select required></select></div><div class="field"><label>Server Name</label><input class="input" name="server" maxlength="50" placeholder="Server 1" required></div><div class="field full"><button class="btn primary" type="submit">Run Web /setup</button></div></form><div class="result" id="setupResult"></div></section>

    <section id="assign" class="card tool-card"><span class="web-command">/assign</span><h3>🏴 Assign Flag</h3><p>Assign an available flag to any non-managed Discord role. Administrator access is required.</p><form id="assignForm" class="form-grid"><div class="field"><label>Setup</label><select class="select setup-select" name="setup" required></select></div><div class="field"><label>Flag</label><select class="select" name="flag" id="assignFlag" required></select></div><div class="field full"><label>Role</label><select class="select" name="role_id" id="roleSelect" required><option>Loading roles…</option></select><div class="role-note">Managed roles and @everyone are excluded automatically.</div></div><div class="field full"><button class="btn primary" type="submit">Assign Flag</button></div></form><div class="result" id="assignResult"></div></section>

    <section id="release" class="card tool-card"><span class="web-command">/release</span><h3>🏳️ Release Flag</h3><p>Return a claimed flag to the available pool.</p><form id="releaseForm" class="form-grid"><div class="field"><label>Setup</label><select class="select setup-select" name="setup" required></select></div><div class="field"><label>Claimed Flag</label><select class="select" name="flag" id="releaseFlag" required></select></div><div class="field full"><button class="btn" type="submit">Release Flag</button></div></form><div class="result" id="releaseResult"></div></section>

    <section id="status" class="card tool-card"><span class="web-command">/flagstatus</span><h3>📊 Flag Status</h3><p>Inspect setup health, channel/message state, available flags, claimed flags, and missing roles.</p><div class="form-grid"><div class="field full"><label>Setup</label><select class="select setup-select" id="statusSetup"></select></div><div class="field full"><button class="btn" id="statusBtn" type="button">Check Status</button></div></div><div class="result" id="statusResult"></div></section>

    <section id="refresh" class="card tool-card"><span class="web-command">/flagrefresh</span><h3>🔄 Refresh Discord Dashboard</h3><p>Force-refresh the stored Components V2 Flag Dashboard in Discord.</p><div class="form-grid"><div class="field full"><label>Setup</label><select class="select setup-select" id="refreshSetup"></select></div><div class="field full"><button class="btn" id="refreshBtn" type="button">Refresh Dashboard</button></div></div><div class="result" id="refreshResult"></div></section>

    <section id="history" class="card tool-card"><span class="web-command">/flaghistory</span><h3>🕘 Flag History</h3><p>View recent claim/release activity for a setup.</p><div class="form-grid"><div class="field"><label>Setup</label><select class="select setup-select" id="historySetup"></select></div><div class="field"><label>Entries</label><select class="select" id="historyLimit"><option>5</option><option selected>10</option><option>20</option></select></div><div class="field full"><button class="btn" id="historyBtn" type="button">Load History</button></div></div><div class="result" id="historyResult"></div></section>

    <section id="delete" class="card tool-card"><span class="web-command">/deletesetup</span><h3>🗑️ Delete Setup</h3><p>Permanently remove a Flag System. This deletes its database records/history and can also remove its Discord channel.</p><form id="deleteForm" class="form-grid"><div class="field full"><label>Setup</label><select class="select setup-select" name="setup" required></select></div><div class="field full"><label><input type="checkbox" name="delete_channel" checked> Also delete the stored Discord flag channel</label></div><div class="field full"><button class="btn danger-btn" type="submit">Delete Setup Permanently</button></div></form><div class="result" id="deleteResult"></div></section>

    <section id="botstatus" class="card tool-card"><span class="web-command">/botstatus</span><h3>🤖 DayZ Manager Status</h3><p>Check Discord, database, latency, uptime, guild count, and command count.</p><button class="btn" id="botStatusBtn" type="button">Load Bot Status</button><div class="result" id="botStatusResult"></div></section>
...
<script>
const GUILD={json.dumps(str(guild.id))},CSRF={csrf},MAPS={maps_json},FLAGS={flags_json};
...
const result=(id,msg,ok=true)=>{{const e=document.getElementById(id);e.textContent=msg;e.classList.add('show');e.style.borderColor=ok?'#285642':'#6d2637'}};
...
function fillMaps(){{document.querySelectorAll('[data-map-select]').forEach(el=>{{el.innerHTML=MAPS.map(m=>'<option value="'+esc(m.key)+'">'+esc(m.name)+'</option>').join('')}})}}
...
async function loadState(){{const d=await api('/state');SETUPS=d.setups;ROLES=d.roles;fillSetups();document.getElementById('roleSelect').innerHTML=ROLES.length?ROLES.map(r=>'<option value="'+esc(r.id)+'">'+esc(r.name)+'</option>').join(''):'<option value="">No assignable roles</option>';renderSetupList()}}
function renderSetupList(){{const e=document.getElementById('setupList');e.innerHTML=SETUPS.length?SETUPS.map(s=>'<div class="setup-row"><div><strong>'+esc(s.map_name+' • '+s.server)+'</strong><div class="tiny">'+esc(key(s))+'</div></div><a class="btn" href="'+esc(s.url||'#')+'">Live Page</a></div>').join(''):'<div class="empty">No Flag Systems yet. Use Web /setup below.</div>'}}
async function updateFlagChoices(){{try{{const a=document.querySelector('#assignForm .setup-select').value;if(a){{const s=unpack(a),d=await api('/flags?map='+encodeURIComponent(s.map)+'&server='+encodeURIComponent(s.server));document.getElementById('assignFlag').innerHTML=d.available.map(x=>'<option value="'+esc(x.flag)+'">'+esc(x.flag)+'</option>').join('')||'<option value="">No available flags</option>'}}const r=document.querySelector('#releaseForm .setup-select').value;if(r){{const s=unpack(r),d=await api('/flags?map='+encodeURIComponent(s.map)+'&server='+encodeURIComponent(s.server));document.getElementById('releaseFlag').innerHTML=d.claimed.map(x=>'<option value="'+esc(x.flag)+'">'+esc(x.flag+' — '+x.role_name)+'</option>').join('')||'<option value="">No claimed flags</option>'}}}}catch(e){{}}}}
document.addEventListener('change',e=>{{if(e.target.classList.contains('setup-select'))updateFlagChoices()}});
document.getElementById('setupForm').addEventListener('submit',async e=>{{e.preventDefault();try{{const f=new FormData(e.target),d=await api('/setup',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(Object.fromEntries(f))}});result('setupResult',d.message);await loadState()}}catch(x){{result('setupResult',x.message,false)}}}});
document.getElementById('assignForm').addEventListener('submit',async e=>{{e.preventDefault();try{{const f=new FormData(e.target),s=unpack(f.get('setup')),d=await api('/assign',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{...s,flag:f.get('flag'),role_id:f.get('role_id')}})}});result('assignResult',d.message);await loadState()}}catch(x){{result('assignResult',x.message,false)}}}});
document.getElementById('releaseForm').addEventListener('submit',async e=>{{e.preventDefault();try{{const f=new FormData(e.target),s=unpack(f.get('setup')),d=await api('/release',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{...s,flag:f.get('flag')}})}});result('releaseResult',d.message);await loadState()}}catch(x){{result('releaseResult',x.message,false)}}}});
document.getElementById('deleteForm').addEventListener('submit',async e=>{{e.preventDefault();const f=new FormData(e.target),s=unpack(f.get('setup'));if(!confirm('Permanently delete '+s.map+' • '+s.server+'? This cannot be undone.'))return;try{{const d=await api('/delete-setup',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{...s,delete_channel:f.get('delete_channel')==='on'}})}});result('deleteResult',d.message);await loadState()}}catch(x){{result('deleteResult',x.message,false)}}}});
document.getElementById('statusBtn').onclick=async()=>{{try{{const s=unpack(document.getElementById('statusSetup').value),d=await api('/status?map='+encodeURIComponent(s.map)+'&server='+encodeURIComponent(s.server));result('statusResult',JSON.stringify(d,null,2))}}catch(x){{result('statusResult',x.message,false)}}}};
document.getElementById('refreshBtn').onclick=async()=>{{try{{const s=unpack(document.getElementById('refreshSetup').value),d=await api('/refresh',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(s)}});result('refreshResult',d.message)}}catch(x){{result('refreshResult',x.message,false)}}}};
document.getElementById('historyBtn').onclick=async()=>{{try{{const s=unpack(document.getElementById('historySetup').value),l=document.getElementById('historyLimit').value,d=await api('/history?map='+encodeURIComponent(s.map)+'&server='+encodeURIComponent(s.server)+'&limit='+l);result('historyResult',d.entries.length?d.entries.map(x=>x.when+' — '+x.flag+' '+x.action+' — '+x.role+' — '+x.actor).join('\\n'):'No history yet.')}}catch(x){{result('historyResult',x.message,false)}}}};
document.getElementById('botStatusBtn').onclick=async()=>{{try{{const d=await api('/botstatus');result('botStatusResult',JSON.stringify(d,null,2))}}catch(x){{result('botStatusResult',x.message,false)}}}};
const tf=document.getElementById('teleporterForm');if(tf)tf.addEventListener('submit',async e=>{{e.preventDefault();try{{const f=new FormData(e.target),r=await fetch('/api/manage/'+encodeURIComponent(GUILD)+'/teleporter',{{method:'POST',headers:{{'X-CSRF-Token':CSRF,'Content-Type':'application/json'}},body:JSON.stringify(Object.fromEntries(f))}});if(!r.ok){{let d;try{{d=await r.json()}}catch{{d={{error:'Generation failed'}}}}throw new Error(d.error)}}const blob=await r.blob(),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='DayZ_Manager_Teleporters.zip';a.click();URL.revokeObjectURL(a.href);result('teleporterResult','✅ Generated both teleporter JSON files.')}}catch(x){{result('teleporterResult',x.message,false)}}}});
fillMaps();fillSetups();loadState();
</script>"""
    return web.Response(text=_page(f"{guild.name} — Control Panel", body, _invite_url(bot), f"Private DayZ Manager controls for {guild.name}."), content_type="text/html", headers={"Cache-Control": "no-store"})
